...
import toJson from 'gulp-to-json';
...
const { COMPATIBILITY, PORT, UNCSS_OPTIONS, PATHS, RESP } = loadConfig();
...
gulp.task('build',
    gulp.series(clean, ex2json , gulp.parallel(index, pages, sass, javascript, templates, angular, jslib, images,resp, copy)));
...
gulp.task('default',
    ...
    function getresolutions(){
       return (JSON.parse(fs.readFileSync('dist/assets/data/config.json', 'utf8'))).resp;
    }


    function resp(done) {
        rimraf(PATHS.dist+'/assets/imgs', done);
        var  resps = getresolutions();
        resps.forEach(function (co) {
    ...
    function ex2json(done) {
            excelsheets2json(__dirname+"/"+PATHS.xlsx,{dest: PATHS.dist + '/assets/data'});
            done();
    }
